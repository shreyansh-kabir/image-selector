...
public class SelectionComponent extends JComponent implements MouseListener, MouseMotionListener,
     ...
     */
    private Color selectionPerimeterColor = Color.BLACK;
    ...
    private void paintLiveWire(Graphics g) {
        PolyLine live = model.liveWire(mouseLocation);
        g.setColor(liveWireColor);
        g.drawPolyline(live.xs(), live.ys(), live.size());
    }
    ...
    private void paintControlPoints(Graphics g, List<PolyLine> segments) {
        int r = 5; // radius of the circles

        for (PolyLine segment : segments) {
            Point p = segment.start(); // first points of each segment
            g.fillOval(p.x - r, p.y - r, 2 * r, 2 * r);

            // note: no need to draw endpoints of the segments since they always coincide with
            // the first point of the next segment (even in the case of the last point of the
            // last segment of the list, which is the same as the first point of the first segment).
        }
    }
    ...
    private void paintMoveGuides(Graphics g, List<PolyLine> segments) {
        // Draw the new preceding segment
        int prevIndex = Math.floorMod((selectedIndex - 1), segments.size());
        // the modulo allows us to loop back if we go less than 0
        PolyLine prev = new PolyLine(segments.get(prevIndex).start(), mouseLocation);
        g.drawPolyline(prev.xs(), prev.ys(), 2);

        // Draw the new succeeding segment
        PolyLine next = new PolyLine(mouseLocation, segments.get(selectedIndex).end());
        g.drawPolyline(next.xs(), next.ys(), 2);
    }
    ...
    public void mouseClicked(MouseEvent e) {
        ...
        if (e.getButton() == MouseEvent.BUTTON1) { // mouse button 1 was clicked
            if (model.state() == NO_SELECTION || model.state() == SELECTING) {
                model.addPoint(e.getPoint());
            }
        } else if (e.getButton() == MouseEvent.BUTTON2) { // mouse button 2 was clicked
            if (model.state() == SELECTING) {
                model.finishSelection();
            }
        } else if (e.getButton() == MouseEvent.BUTTON3) { // mouse button 3 was clicked
    ...
    public void mousePressed(MouseEvent e) {
        if (e.getButton() == MouseEvent.BUTTON1 && model.state() == SELECTED) {
            selectedIndex = model.closestPoint(getMousePosition(),
