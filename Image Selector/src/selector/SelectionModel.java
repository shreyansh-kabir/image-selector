...
public abstract class SelectionModel {
    ...
    public int closestPoint(Point p, int maxDistanceSq) {
        ...
        int closestIndex = 0;
        Point closestP = start;

        ListIterator<PolyLine> li = selection.listIterator();
        while (li.hasNext()) {
            int currentIndex = li.nextIndex();
            Point currentP = li.next().start();
            if (p.distanceSq(currentP) < p.distanceSq(closestP)) {
                closestIndex = currentIndex;
                closestP = currentP;
            }
        }

        if (p.distanceSq(closestP) < maxDistanceSq) {
            return closestIndex;
        } else {
            return -1;
        }
    ...
    protected void undoPoint() {
        if (selection.isEmpty()) {
            ...
            if (state == SELECTED) {
                setState(SELECTING);
                // no need to fire property change for `state` because setState() already does that.
            }
        }
    }
